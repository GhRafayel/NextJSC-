	...
	<div  className={`${cntUser?.theme ?? true ?  "bg text-gray-900" : "bg-gray-200"} w-full h-screen flex items-center justify-center px-4`} >
		...
		<div className="w-full max-w-md mx-auto p-6 glass rounded-2xl">
			...
			<div className="w-full text-center my-3">
				<h2 className="text-4xl font-bold"> {LENUAGE.Auth.login.title} </h2>
			</div>
			...
			<form onSubmit={ async (e) => { 
				...
				.then( async (res) => res.ok ? (ChangingCallback(undefined, "me"), router.push("/")) : (console.log(res), setLogin(LENUAGE.Auth.login.error)))
			}}
            >
				{loginData.map((item, i) => ( <FormInputs key={i} item={item} placeholder={LENUAGE.Auth.fields[item.name as keyof typeof LENUAGE.Auth.fields]}/>) )}
			...
			<div className="flex items-center gap-3 px-6 my-2">
				...
				<span className="text-xl">{LENUAGE.Auth.login.or}</span>
				<div className="flex-1 h-px bg-gray-700/40" />
			</div>
